...
class Response extends \Magento\Framework\DataObject {
    ...
    protected function parseRawResponse(\Zend\Http\Response $response) {
        if ($response->isSuccess()) {
            ...
            $this->_searchHelperData->log(\Zend\Log\Logger::ERR, sprintf("Unsuccessful HTTP response: %s %s", $response->getStatus(), $response->responseCodeAsText($response->getStatus())));
        }
